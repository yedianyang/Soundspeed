...
一台 Windows 11 + RTX 3070 Ti、已建好 Python 3.13 venv、但还没装 llama-cpp-python 的机器，探测结果大致长这样（脱敏样例，路径和值按本机会不同）：
...
## 2. 装 Python 3.13 + 建并激活 venv

从 python.org 装 Python 3.13（勾「Add to PATH」），然后：
...
venv 用的 Python 版本（这里是 3.13）决定了能装哪个 cp 标签的 wheel——但这只对标准 CPython-ABI wheel 成立：那种 wheel 在 3.13 venv 里只能装 cp313 的，装错标签 pip 会拒绝或回退去编译源码。§3 首选的 dougeeai py3-none（ctypes）wheel 不绑 Python 版本，不受此限。选 3.13 是因为它对标准 ABI 和 py3-none 两类 wheel 覆盖都全（详见 §3 / §8）。各台机的 venv 互相独立，mac 那边仍用 3.14，不冲突。
...
- **v0.3.20 是 `py3-none`**（ctypes 实现，**Python 版本无关**，3.13 / 3.14 都能装），按 CUDA+sm 分。
...
```
llama_cpp_python-0.3.16+cuda12.1.sm86.ampere-cp313-cp313-win_amd64.whl
```
...
注意：首选的 v0.3.20 是 py3-none，不绑 Python 版本，所以这一步不用纠结 cp 标签和 venv 的 Python 对不对——只有走备选的 v0.3.16（或下面自编译）那种标准 CPython-ABI wheel 才要 cp313 对齐 3.13 venv。
...
- **cp 标签必须对齐 Python 版本（仅限标准 ABI wheel）**：只对标准 CPython-ABI wheel 成立——dougeeai v0.3.16、rookiemann、自编译产物这类，cp313 wheel 只能进 3.13 venv，装错 pip 会拒装或回退编译。dougeeai **v0.3.20 是 py3-none（ctypes），Python 版本无关，不受此限**，3.13/3.14 都能装。
...
- **cp314 的 Windows 标准 ABI CUDA wheel 没有**：标准 CPython-ABI 的 cp314 CUDA wheel 目前没有现成的——rookiemann 和 dougeeai v0.3.16 都只到 cp313。但 dougeeai v0.3.20 是 py3-none，理论上 3.14 也能装（未实测）。选 3.13 是图稳：cp313 标准 ABI wheel 和 py3-none wheel 都覆盖得到，不是因为 3.14 完全不可能。这也是项目把 `requires-python` 从 3.14 放宽到 `>=3.13` 的原因——给 Windows 留 3.13 这条覆盖最全的路。所以 Windows 落 3.13，mac 仍用 3.14，各机 venv 独立。
