...
> **实现勘误（2026-06-06，合并 PR #39 前审计发现，非阻塞）**：D-QP-01 / §3.1 / §5 伪码写的 `run_tool_loop(messages, toolset)`（toolset 作参数为 option 2 留口）**v1 未按字面落地**——实际签名是 `run_tool_loop(messages, *, service, dal, max_hops, timeout)`，工具集硬写为模块常量 `_QP_TASK = "query_session"`（`qp_query.py:28`），且 `infer`/`infer_tool` 经 `task_type` 索引 `TASK_CONFIG` 取 tools、无 `tools`/`toolset` kwarg。v1 单 toolset 单入口正确（实证通过）。**option 2 迁移门未关，但成本上修一项**：除「砍分类器 + 把 record_note 加进 toolset + 入口指向统一循环」外，还需「给 `run_tool_loop` 加 `task_type` 形参（default `"query_session"`）并透传到 `infer`/`infer_tool`」。照 D-QP-01 字面排期者勿漏此项。
