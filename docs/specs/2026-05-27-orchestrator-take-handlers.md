...
⚠ **MVP 实现挂起**：逐条 try/except（单条 FK 失败跳过其余继续写入）在 MVP 内暂未实现。实际实现使用单事务批量写入，任一行 FK 违约会导致整批回滚。待 1.I 集成测试发现有真实 FK 违约场景时再补充。
...
5. 取最后 5 条（`[-5:]`，最近的历史 take）。
6. 每条截断至 800 字符：`note[:800]`。
7. 返回 `list[str]`，可为空列表。
...
| `test_previous_notes_assembly` | 建 5 个已有 take（各有 script_diff），组装后 previous_notes 取最近 5 条，每条不超 800 字符 |
