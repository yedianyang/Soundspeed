...
版本：v0.4
日期：2026-05-28
状态：spike 执行完成，Lead 评审通过，已合并 main（PR #2 + 后续 n_ctx 升级）

变更记录：
- v0.4（2026-05-28）：n_ctx 起步值从 4096 升到 8192（§11.3）。理由：1.G v0.2 L2 pipeline 实测短 take 输出 ~2000 token，长 take 场景（剧本 100+ 行 / 5+ 分钟转录）需要更大 context；Q4_K_M @ n_ctx=8192 RSS ~6.5 GB，M1 Max 16 GB 余量充足（§11.2 表格已含 n_ctx 8192 临界对照）。开放问题 §9.3 关闭：8192 验证为新基线。
...
3. ~~**n_ctx 上限**：4096 是否够覆盖 SP（script_parse）的剧本输入？长剧本可能要 8192+，context 越大内存越大，需在 §5 加测一组。~~ **v0.4 关闭**：1.G v0.2 实测 4096 在长 take（剧本 100+ 行）下不够，n_ctx 升到 8192 作为新基线（见 §11.3）。
...
- 配置：`n_ctx=8192`（v0.4 升级，原 4096 在 1.G v0.2 长 take 下不够）、`n_gpu_layers=-1`、`seed=42`、Metal 全卸载
