...
版本：v0.2（Lead 评审决议落实，进入开发）
日期：2026-05-27
状态：v0.2（Lead 评审决议落实，进入开发）
owner：境熙
...
选用 `unicode61 remove_diacritics 2`，理由：

- `unicode61`：SQLite 内置 tokenizer，按 Unicode 分词规则切词，支持拉丁字母、数字、常见标点，满足英文剧本台词检索。
- `remove_diacritics 2`：规范化变音符号（如 é → e），搜索「不加音调」也能匹配带音调词。

中文场景的限制：unicode61 默认将 CJK 字符视为独立 token（每个汉字一个 token）。对中文台词做字面子串检索可行，但不支持词粒度分词（例如「我不想走」搜「不想」能匹配，但搜「我不」也能部分匹配）。精确中文词粒度检索需要 jieba 预分词后再喂 FTS5，本期不引入（开放问题 §8.2 有记录）。语义级中文检索由 QP Pipeline 兜底，不依赖 FTS5。

兜底方案：若 `unicode61` 在中文测试中命中率不足，降级到 `simple` tokenizer（完全按空白切词，适合空格分隔的手工分词文本）。
...
def apply_migrations(db_path: Path) -> None:
    ...
    try:
        ...
        # 服务启动时清空 active_observers（easy-volatile 数据，不保留跨重启）
...
- `active_observers` 清空在 `apply_migrations` 里执行，确保每次启动时连接状态从空白开始（system-architecture §10.3）。
...
DAL 接口签名总数：22 个公开方法（含数据类定义 4 个，不计 `__init__`）。
...
- `test_active_observers_cleared_on_startup` — `apply_migrations` 清空 active_observers
...
- `test_insert_take_event_invalid_json_raises` — 非法 JSON payload 触发 CHECK 约束异常
...
- `test_match_script_line_fts5_chinese` — FTS5 MATCH 中文字符级子串查询（unicode61）
...
`unicode61` tokenizer 对中文按汉字粒度分 token，字面子串检索可用（搜「不想走」能匹配含「不想走」的台词），但词粒度检索不支持（无法精确匹配「不想」作为完整词）。

若中文剧本的检索召回率不足，方案选项：
- 写入 `script_lines` 前用 jieba 预分词，存储空格分隔的分词文本到单独的 `text_segmented` 列，FTS5 索引该列。代价是增加写入复杂度和存储量。
- 完全不依赖 FTS5 做语义检索，由 QP Pipeline（Gemma 4 E4B）做语义匹配；FTS5 只用于精确关键词兜底。

本期不引入 jieba，记录问题留后续评估。
...
*本 spec 状态为 v0.1 草稿，待 Lead（境熙）评审后升版。评审重点：字段是否覆盖上游 spec 所有字段来源、CHECK 约束完整性、DAL 签名是否满足 contract C2 强制接口、开放问题是否遗漏。*
