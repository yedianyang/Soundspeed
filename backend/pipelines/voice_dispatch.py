...
注意：文本 QP 主线已默认 native 回喂（B2，Task 14 定位 bug 在 Llava15 路径、
带 tools 走 GGUF 模板安全）；语音续跳因混合序列未 A/B，显式钉 native_toolfeed=False，
迁移归 Task 16。回喂格式：assistant 喂 hop A 原始 content + user 喂工具返回文本。
...
async def _handle_query_branch(
    ...
    *,
    ...
    未 A/B，显式钉 native_toolfeed=False，迁移归 Task 16。
    回喂格式（text 回退模式）：
    ...
    try:
        ...
        answer = await run_tool_loop(
            ...
            service=service,
            dal=dal,
            native_toolfeed=False,  # B2:语音续跳是「纯文本 assistant 帧+native 帧」混合序列,未 A/B,钉文本模式待 Task 16 专项迁移
        )
    except Exception as exc:
