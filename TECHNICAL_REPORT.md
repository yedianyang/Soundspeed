...
- **语音指令驱动 agent（片场最顺手的用法）**：手机上按住说一句话，原始语音经局域网直达后端的 Gemma 4，由它**听懂意图 → 选工具 → 取参 → 执行**（如口述「这条 keep」完成标记、「第三场拍了几条」即时作答）——把 §5.5 / §5.6 的语音多模态与 agentic 函数调用，落成片场最自然的「对着手机说」交互。
...
- **移动端（呼应大赛「最佳移动应用」方向）**：纯 Web 前端 + 局域网 HTTPS，让手机成为片场遥控台，手机语音指令直接驱动 Gemma agent 完成标记/查询（见 §八）。
