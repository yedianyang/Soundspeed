...
- **语音指令驱动 agent（片场最顺手的用法）**：手机上按住说一句话，原始语音经局域网直达后端的 Gemma 4，由它**听懂意图 → 选工具 → 取参 → 执行**（如口述「这条 keep」完成标记、「第三场拍了几条」即时作答）——把 §4.5 / §4.6 的语音多模态与 agentic 函数调用，落成片场最自然的「对着手机说」交互。
