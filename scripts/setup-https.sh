...
VITE_API_BASE=https://${SITE}
EOF

# --- 6. 渲染本机 Caddyfile ---
echo "▶ 渲染 ./Caddyfile（gitignore）"
sed "s/{{SITE}}/${SITE}/g" Caddyfile.template > Caddyfile
...
下一步：
  ...
  3. 起 Caddy：  sudo caddy run --config ./Caddyfile     # 443 是特权端口
  4. 手机同 Wi-Fi 开：https://${SITE}/admin   （在「服务器连接」手填 token：devtoken）
